...
class EntitiesDictExtrator(object):
    ...
    def get_sentence_common_info(self, sentence_struct_info: dict, entities_index_dict: dict):
        ...
        fieldname2pattern = {
            "occurrence_dates": r"<发生时间>",
            "disclosed_dates": r"<披露时间>",
            # "deal_types": r"<交易类型>|<交易类型><金额><交易类型>"
            "deal_types": r"<交易类型>"
        }
        sentence_struct_info["occurrence_dates"] = {}
        sentence_struct_info["disclosed_dates"] = {}
        sentence_struct_info["deal_types"] = {}
        for fieldname, pattern in fieldname2pattern.items():
            matches = re.finditer(pattern, entities_sent)
            for m in matches:
                sp = m.span()
                sentence_struct_info[fieldname][get_field_value(
                        sent, entities_index_dict, sp)] = sp
        if len(sentence_struct_info["deal_types"]) == 0:
            dt_reobj = re.compile(
                r"(?:(?:(?:Pre-|pre-)?[A-H]\d?|天使|种子|新一|上一?|本|此|该|两|首)(?:\+)?(?:轮|次)(?:融资)?)|天使投资")
            sentence_struct_info["deal_types"].update({m[0]:(-1, -1) for m in dt_reobj.finditer(sent)})

# 《科创板日报》（北京，记者郭辉）讯，今日（24日），ESG信息服务商妙盈科技正式宣布获得国泰君安国际私募股权基金和GIC（新加坡政府投资公司）的联合B+轮投资。
    def convert_str_to_entities_str(self, sent: str):
        # 请求 ner
        ...
        entities_sent, entities_index_dict, labels_unused, labels_value = "", {}, [], []
        alias = {}
        replaced_list = []
        # 标签替换前后首索引差值
        idx_diff = 0
        ...
        for i, label in enumerate(labels_indexes):
            ...
            # 处理非法交易类型
            if label[2] == "交易类型" and not self.validate_deal_type_reobj.search(re.sub(r"\s",'', label_content)):
                replaced_list.append(label_content)
                start = label[1]
                labels_unused.append([label_content, label[2]])
                continue
            # 生成标签索引映射
        ...
        return {
            ...
            "entities_index_dict": entities_index_dict,
    ...
    def adjust_field(self, sentence_struct_info):
        ...
        for mr in match_result:
            ...
            if "dates" in mr_struct:
                disclosed_dates = []
                occurrence_dates = []
                for d in mr_struct["dates"]:
                    if d in disclosed_dates:
                        disclosed_dates.append(d)
                    else:
                        occurrence_dates.append(d)
                # 待解决
                if len(disclosed_dates) > 0:
                    mr_struct["disclosed_date"] = sorted(
                        disclosed_dates, key=lambda i: (len(i), i), reverse=True)[0]
                if len(occurrence_dates) > 0:
                    mr_struct["occurrence_date"] = sorted(
                        occurrence_dates, key=lambda i: (len(i), i), reverse=True)[0]
                del mr_struct["dates"]
            else:
                # 待解决
                if len(sentence_struct_info["disclosed_dates"]) > 0:
                    for k, v in sentence_struct_info["disclosed_dates"].items():
                        mr_struct["disclosed_date"] = k
                        total_labels_used.add(v)

                if len(sentence_struct_info["occurrence_dates"]) > 0:
                    for k, v in sentence_struct_info["occurrence_dates"].items():
                        mr_struct["occurrence_date"] = k
                        total_labels_used.add(v)
    ...
    def __call__(self, sent):
        ...
        sentence_struct_info = self.convert_str_to_entities_str(sent)
        if not sentence_struct_info:
            return
        entities_sent = sentence_struct_info["entities_sent"]
        entities_index_dict = sentence_struct_info["entities_index_dict"]
        self.get_sentence_common_info(sentence_struct_info, entities_index_dict)
